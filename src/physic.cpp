      ...
      for(Inter j = std::next(i); j != end; ++j) {
        if((*j)->is_hitable()) {
          for(auto& other_box : (*j)->bounding_box_list_) {
            ...
            CylinderBB* c1 = (s1 == nullptr && o1 == nullptr)
                                 ? dynamic_cast<CylinderBB*>(other_box)
                                 : nullptr;
            ...
            for(auto& h_box : (*i)->bounding_box_list_) {
              ...
              CylinderBB* c2 = (s2 == nullptr && o2 == nullptr)
                                   ? dynamic_cast<CylinderBB*>(h_box)
                                   : nullptr;
              ...
              if(s1 != nullptr && s2 != nullptr) {
                ...
                collision(*s1, *o2);
              } else if(s2 != nullptr && o1 != nullptr) {
                collision(*s2, *o1);
              } else if(s1 != nullptr && c2 != nullptr) {
                collision(*c2, *s1);
              } else if(s2 != nullptr && c1 != nullptr) {
                collision(*c1, *s2);
              }
            }
          }
...
void Physic::collision(OBB&, OBB&) {
  // TODO
}

void Physic::collision(CylinderBB&, CylinderBB&) {
  // TODO
}
void Physic::collision(CylinderBB&, OBB&) {
  // TODO
}

void Physic::collision(CylinderBB& a, SphereBB& b) {
  auto closest_point_on_line
      = [](const GLVector<XYZW>& line_start, const GLVector<XYZW>& line_end,
           const GLVector<XYZW>& point)
            ->GLVector<XYZW> {
    const GLVector<XYZW> es = line_end - line_start;
    return GLVector<XYZW>(line_start
                          + es * (((point - line_start) * es) / es.Length2()));
  };
